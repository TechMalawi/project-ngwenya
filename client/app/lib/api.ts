...
export class StrapiAPI {
	...
	async getArticles(params?: {
		...
		return this.request<StrapiResponse<StrapiArticle[]>>(
	...
	async getAuthors(): Promise<StrapiResponse<StrapiAuthor[]>> {
		...
		return this.request<StrapiResponse<StrapiAuthor[]>>(
...
export function getStrapiImageUrl(
	...
	if (format && image.formats?.[format]?.url) {
		...
		return isAbsoluteUrl(formatUrl) ? formatUrl : `${baseUrl}${formatUrl}`;
	}

	return isAbsoluteUrl(image.url) ? image.url : `${baseUrl}${image.url}`;
}
